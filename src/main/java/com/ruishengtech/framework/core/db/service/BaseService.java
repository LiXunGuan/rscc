...
@Service
@Transactional
public class BaseService extends JdbcService {
    ...
    public void save(Object o, String[] excludeFieldName) {
...
//        getTemplate(Template.CACHE).update(deleteSql.toString(), params.toArray());
	...
	public <T> PageResult<T> queryPage(final ISolution sysConfSolution, final ICondition condition, Class<T> clazz) {
        ...
        if(orders != null){
        	for (String[] o : orders) {
        		String by = tableDefinition.getFieldToColums().get(o[0]);
        		pageSql.append(by==null?o[0]:by).append(" ").append(o[1]).append(",");
        	}
        }
        ...
        return new PageResult(count.size()==0?0:count.get(0), count.size()==0?0:count.get(0), list);
    }
    
    
    @SuppressWarnings({ "unchecked", "rawtypes" })
	public <T> PageResult<T> queryPageNumLimit(final ISolution sysConfSolution, final ICondition condition, Class<T> clazz) {
...
//        	pageSql.append(tableDefinition.getFieldToColums().get(o[0])).append(" ").append(o[1]).append(",");
        ...
        List<T> list = queryBean(new BeanHandler() {
            ...
            public void doSql(final StringBuilder sql, final List<Object> params) {
                ...
                if (condition instanceof Page) {
                    sql.append(" limit ? ");
                    params.add(((Page) condition).getNumLimit());
                }
            }
        }, clazz);
...
//===============
    ...
    public <T> List<T> getAllById(final Class<T> c) {
        return getAllById(c, null, new ArrayList<Object>());
    }
    
    public <T> List<T> getAllById(final Class<T> c, String where, final List<Object> ps) {

        String tableName = DbContext.getInstance().getTablesMap().get(c).getTableName();
        final String sqll = " select * from " + tableName + "  " + (StringUtils.isBlank(where) ? "" : where+" order by id asc");
    ...
    public <T> T getById(Class<T> clazz, Long id) {
        ...
        List<Object> params = new ArrayList<Object>();
        params.add(id);
        List<T> ret = getAllById(clazz, " where id =?", params);
        if (ret == null || ret.size() < 1) {
            return null;
        } else {
            return ret.get(0);
	...
	public <T> List<T> getBeanList(Class<T> clazz, final String whereSql, final Object... finalParams) {
        ...
        final String finalSql = " SELECT * FROM " + tableName + " WHERE 1 = 1  " + (StringUtils.isBlank(whereSql) ? "" : whereSql);
	...
	@SuppressWarnings("unchecked")
	public <T> List<T> getBeanListWithSql(Class<T> clazz, final String finalSql, final Object... finalParams) {
...
//            saveCacheBean(o, tableDefinition);
    ...
    public void update(String tableName, Object o) {
    	update(tableName, o, new String[]{});
    }
    
    public void update(String tableName, Object o, String[] excludeFieldName) {
...
//            getTemplate(Template.CACHE).update(updateSql.toString(), params.toArray());
    ...
    public void deleteById(String tableName, Class c, UUID uuid) {
