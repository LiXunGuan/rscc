	...
	<div id="agentnoticeinfoDiv"></div>
	<div id="mainContent" style="">
	<!-- HEADER -->
		<!-- END RIBBON -->
		<!-- widget grid -->
		<section id="widget-grid-agentnoticeinfo" class="">
			<div class="row">
...
<!-- 					    						</div> -->
										...
										<table id="dt_basic_agentnoticeinfo" class="table table-striped table-bordered table-hover"  data-order='[[ 0, "asc" ]]' width="100%"></table>
									</div>
								</div>
							</div>
						</div>
	...
	<script type="text/javascript">
		...
		function initTable(){
			oTable = $('#dt_basic_agentnoticeinfo').dataTable({
...
// // 							   }
// 							   return "&nbsp;&nbsp;<a onclick=deleteItem('"+full.uid+"');>删除</a>"; 
// 						   }
// 					   }
					],
			});
			        ...
			        if (ButtonPressed === "Yes") {
			            ...
			            $.post(url,{agentNoticeUUid:uuid},function(data){
			                if(data.success){
			                    ...
			                    $('#dt_basic_agentnoticeinfo').DataTable().ajax.reload(null,false);;
			                }else{
			                    $.smallBox({
			                        title : "发布提醒",
