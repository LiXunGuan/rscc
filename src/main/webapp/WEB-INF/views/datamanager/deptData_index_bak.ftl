			 ...
			 var dt = $('#dt_basic_deptdata').DataTable({
				...
				"columns" : [
					   ...
					   { "title" : "部门内数据量", "data" : "null","render":function(data,type,full){
						   if(full.dataCount == 0)
						     return "<a style='color:red;font-size:15px;' onclick=getDataCount('"+full.batchname+"','"+full.dataBatchUuid+"','"+full.departmentUuid+"','"+full.isLock+"');>"+ full.dataCount +"</a>";
						   else
						     return "<a onclick=getDataCount('"+full.batchname+"','"+full.dataBatchUuid+"','"+full.departmentUuid+"','"+full.isLock+"');>"+ full.dataCount +"</a>";
					   }}, 
					   ...
					   { "title" : "意向客户量", "data" : "null","render":function(data,type,full){
						   return "<a onclick=getIntentCount('"+full.batchname+"','"+full.dataBatchUuid+"','"+full.departmentUuid+"');>"+ full.intentCount +"</a>";
					   }},
					   { "title" : "成交客户量", "data" : "customerCount","render":function(data,type,full){
						   return "<a onclick=getCustomerCount('"+full.batchname+"','"+full.dataBatchUuid+"','"+full.departmentUuid+"');>"+ full.customerCount +"</a>";
					   }},
					   ...
					   { "title" : "黑名单号码量", "data" : "null","render":function(data,type,full){
						   return "<a onclick=getBlackListCount('"+full.batchname+"','"+full.dataBatchUuid+"','"+full.departmentUuid+"');>"+ full.blacklistCount +"</a>";
					   }},
					   ...
					   { 
						   ...
						   "render": function(data,type,full){
							   ...
							   "<a onclick=getbackItem('"+full.dataBatchUuid+"','"+full.departmentUuid+"','"+(full.ownCount)+"');>回收</a>";
						   }
					   }
					],
			});
...
// 			var checked = $("#" + id).is(":checked");
		...
		function getbackItem(dataBatchUuid, deptUuid, ownCount){
			$('#deptDataDiv').empty();
	        var url = getContext() + "deptdata/dept/collectData";
