...
// 							   var dom = $('<div><label class="toggle"><input type="checkbox" name="checkbox-toggle" id=' + '"' + full.uid + '"' +  ' onchange="changeState(this.id)"><i data-swchon-text="开启" data-swchoff-text="关闭"></i></lable></div>')
					   ...
					   { "title":"实时并发数","data":"concurrency","orderable":true,"defaultContent":"无"},
					   ...
					   { 
						   ...
						   "render": function(data,type,full){
							   ...
							   if (full.stat == undefined || full.stat == "0") {
								   change = "&nbsp;&nbsp;&nbsp;&nbsp;<a onclick='update${model?cap_first}(\""+full.uid+"\");'>修改</a>";
							   } else {
								   change = "";
							   }
...
// 							   <a onclick="' + "review${model?cap_first}(\'"+full.dataTable+"\');\">查看数据</a>&nbsp;&nbsp;&nbsp;&nbsp;
							   return "<a onclick='review${model?cap_first}(\""+full.groupcall_id+
							   		   "\");'>查看数据</a>&nbsp;&nbsp;&nbsp;&nbsp;<a onclick='dataAllocate(\""+full.groupcall_id+
		...
		function resetData(groupcallId, type) {
			$.post(getContext()+'data/groupCall/resetData',{groupcallId:groupcallId,type:type},function(data){
				if(data.success){
					$.smallBox({
						title : "操作成功",
		...
		function dataCollection(uuid){
			if(uuid!=""){
				$.post(getContext() + "data/${model}/removeData",{"uuid":uuid},function(data){
					$("#userContent").append(data);
				});
			}
		}
		...
		function add${model?cap_first}(){
			$.post(getContext() + "data/${model}/a",function(data){
				$("#userContent").append(data);
			});
		}		
		...
		function changeState(obj){
			...
			$.post(getContext() + "data/${model}/changeStat",{uuid:obj,stat:checked},function(data){
				if(data.success){
					if(data.stat){
