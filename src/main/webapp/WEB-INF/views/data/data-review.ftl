	...
	<div class="modal-dialog" style="width: 75%;">
		<div class="modal-content">
			<div class="modal-header">
				...
				<h4 class="modal-title">数据预览——当前人员：${entry.projectName}，共有数据：${entry.dataCount}条，已完成数据：${entry.userCount}条</h4>
			</div>
								...
								<div class="widget-body no-padding">
									<div class="widget-body-toolbar">
										<div class="smart-form">
											<div class="row">
												<section>
													<input type="hidden" name="currentUuid" id="currentUuid" value="${currentUuid!''}" />
													...
													<label class="input col col-1">
														<input type="text" name="dataSource" id="dataSource" value="" />
													</label>
...
// 		}
		...
		function docheckall(){
		    ...
		    var url = getContext()+'data/project/checkDataAll';
...
// 		        itemCheckList[uid]=$("#ck"+uid).parent().next().text()
		...
		$("#currentProject").change(function(){
	    	if($("#currentProject").val()!=""){
    			$.post(getContext() + "data/project/tasks",{"projectUuid":$("#currentProject").val()},function(data){
            		$("#currentTask").empty();
		...
		$("#currentTask").change(function(){
	    	if($("#currentTask").val()!=""){
    			$.post(getContext() + "data/project/items",{"projectUuid":$("#currentProject").val(),"taskName":$("#currentTask").val()},function(data){
    				for(var i in data.items){
    					itemCheckList[data.items[i]]=true;
    				}
    				$("table.dataTable").DataTable().ajax.reload(null,false);;
		...
		function addCallTimes(uuid) {
			$.post(getContext()+'data/project/addCallTimes', {taskTable:$("#currentUuid").val(),taskUuid:uuid}, function(data) {
				if(data.success){
					$.smallBox({
						title : "操作成功",
						content : "<i class='fa fa-clock-o'></i> <i>操作成功...</i>",
		...
		function deleteOne(uuid) {
			$.SmartMessageBox({
				...
				if (ButtonPressed === "Yes") {
					$.post(getContext()+'data/project/revertOneData',{'dataUuid':uuid,'projectUuid':$("#currentUuid").val()},function(data){
						if(data.success){
							$.smallBox({
								title : "操作成功",
								content : "<i class='fa fa-clock-o'></i> <i>操作成功...</i>",
		...
		function importOne(uuid) {
			if($("#currentTask").val()!=""){
				...
				$.post(getContext()+'data/project/importToProject',data,function(data){
					if(data.success){
						$.smallBox({
							title : "操作成功",
							content : "<i class='fa fa-clock-o'></i> <i>操作成功...</i>",
...
<script src="${springMacroRequestContext.contextPath}/public/js/data/data-review.js"></script>
