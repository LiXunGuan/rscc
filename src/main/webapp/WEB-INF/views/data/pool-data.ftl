...
//		        checklist[uid]=$("#ck"+uid).parent().next().text()
				...
				"columns" : [
					{
					   ...
					   { 
						   ...
						   "data" : "null",
						   "render": function(data,type,full){
							   var str = '<a onclick="' + "review${model?cap_first}(\'"+full.dataTable+"\');\">查看数据</a>&nbsp;&nbsp;&nbsp;&nbsp;<a id=" + full.dataTable + ' onclick="' + "import${model?cap_first}(\'"+full.uid+"\');\">分配数据</a>&nbsp;&nbsp;&nbsp;&nbsp;<a onclick='collection${model?cap_first}(\""+full.uid+"\");'>回收数据</a>&nbsp;&nbsp;&nbsp;&nbsp;<a onclick='move${model?cap_first}(\""+full.uid+"\");'>移动数据</a>&nbsp;&nbsp;&nbsp;&nbsp;<a onclick='distinct${model?cap_first}(\""+full.uid+"\");'>去重</a>&nbsp;&nbsp;&nbsp;&nbsp;<a onclick='update${model?cap_first}(\""+full.uid+
			...
			$.post(getContext() + "data/${model}/add",function(data){
				...
				$('#fileupload').fileupload({
			    	...
			    	add: function (e, data) {
			    		...
			    		$("#msubmita").off("click");
			 			$("#msubmita").on("click",function(){
			 				$.post(getContext() + '/data/data/save',{containerName:$("#newDataName").val()},function(newdata){
			 					if(newdata.success){
		                            $.smallBox({
		                                title : "操作成功",
		                                content : "<i class='fa fa-clock-o'></i> <i>操作成功</i>",
			        ...
			        progressall: function (e, data) {
			            ...
			            $('#progress' + $("#dataUuid").val()).parent().prev().children().text("已完成" + progress + '%');
			        },
		...
		function review${model?cap_first}(uuid) {
			$.post(getContext() + "data/${model}/importModal",{"dataTable":uuid},function(data){
				$("#userContent").append(data);
			});
		}
		...
		function delete${model?cap_first}(uuid){
			$.SmartMessageBox({
				title : "删除",
				content : "<h1>该操作将删除池中数据，并删除已分配的数据，且无法找回，确定执行？</h1><h1>如果确定要删除，请在下面输入框中输入\"立即删除\"四个字，再点击Yes按钮</h1>",
				buttons : "[No][Yes]",
				input : "text",
				placeholder : "请输入“立即删除”四个字，确认删除",
				inputValue : ""
		...
		function import${model?cap_first}(nowId){
			$.post(getContext() + "data/data/allocate",{"containerId":nowId},function(data){
				$("#userContent").append(data);
			});
		}	
		
		function collection${model?cap_first}(nowId){
			$.post(getContext() + "data/data/collection",{"containerId":nowId},function(data){
				$("#userContent").append(data);
			});
		}	
		
		function move${model?cap_first}(nowId){
			$.post(getContext() + "data/dataPool/move",{"containerId":nowId},function(data){
				$("#userContent").append(data);
			});
		}
